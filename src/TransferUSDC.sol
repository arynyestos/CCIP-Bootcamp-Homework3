...
contract TransferUSDC is OwnerIsCreator {
    ...
    event UsdcTransferred(
        bytes32 messageId, uint64 destinationChainSelector, address receiver, uint256 amount, uint256 ccipFee
    );
    ...
    function transferUsdc(uint64 _destinationChainSelector, address _receiver, uint256 _amount, uint64 _gasLimit)
        external
        onlyOwner
        ...
        returns (bytes32 messageId)
    {
        ...
        Client.EVMTokenAmount memory tokenAmount = Client.EVMTokenAmount({token: address(i_usdcToken), amount: _amount});
        ...
        if (ccipFee > i_linkToken.balanceOf(address(this))) {
            revert NotEnoughBalance(i_linkToken.balanceOf(address(this)), ccipFee);
        }
        ...
        emit UsdcTransferred(messageId, _destinationChainSelector, _receiver, _amount, ccipFee);
    }
